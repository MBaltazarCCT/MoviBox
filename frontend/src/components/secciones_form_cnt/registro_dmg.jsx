...
function Registro_dmg({index}) {
  ...
  const [nuevoCandado, setNuevoCandado] = useState(false)
      ...
      <Row className="pt-0">
        {Object.keys(images).map((key) => (
          <Row key={key} className="gy-2">
            {key === "Foto_candado_adicional" ? (
              <>
                  <Button
                    variant="link"
                    className="d-flex"
                    onClick={() => setNuevoCandado(!nuevoCandado)}
                  >
                    {nuevoCandado ? (<p>- Eliminar candado</p>) : (<>+ Agregar nuevo candado</>)}
                  </Button>
