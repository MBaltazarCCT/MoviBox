...
export function transformarData(datos) {
  ...
  datosNuevos.estatus = "PENDIENTE";
  datosNuevos.acceso = "Llegada";
  datosNuevos.user = "Max";
  return datosNuevos;
};
...
// transformar los datos de unidades

export function unidades_procesadas(unidades) {
   const unidadesNuevo = unidades.filter((x) => x[0] !== "")  // Filtra los vacíos
  .map((x) => x[0]);
  return unidadesNuevo;
};
