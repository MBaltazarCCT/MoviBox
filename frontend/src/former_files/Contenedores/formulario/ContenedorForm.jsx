...
import { FetchFoto } from "./FetchOperadores";
...
const FormDatosLlegada = ({
  ...
  const foto = watch("operador");

  useEffect(() => {
    const fetchFoto = async () => {
      setIsLoading(true);
      if (foto) {
        const res = await FetchFoto(foto);
        setPreviewImage(res);
      }
      setIsLoading(false); // Move this inside the fetchFoto function
    };
    fetchFoto();
  }, [foto]);
              ...
              <Form.Group controlId={`operador`}>
                ...
                {errors[`operador`] && (
                  <p className="text-danger">
                    Se requiere seleccionar un cliente
                  </p>
                )}
              </Form.Group>
            </Col>
